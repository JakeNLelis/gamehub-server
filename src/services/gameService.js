...
class GameService {
  ...
  async syncGames() {
    try {
      ...
      if (!apiGames.length) {
        console.log("⚠️  No games found in API response");
        return { success: false, message: "No games found in API" };
      }

      // Get existing games from database
      const existingGames = await Game.findByExternalIds(
        apiGames.map((game) => game.id)
      );

      const existingGameIds = new Set(
        existingGames.map((game) => game.externalId)
      );
      ...
      for (const apiGame of apiGames) {
        try {
          const gameData = this.transformGameData(apiGame);

          if (existingGameIds.has(apiGame.id)) {
            // Update existing game (preserve ratings)
            const existingGame = existingGames.find(
              (g) => g.externalId === apiGame.id
            );
            await Game.findByIdAndUpdate(existingGame._id, {
              ...gameData,
              averageRating: existingGame.averageRating,
          ...
          console.error(
            `❌ Error processing game ${apiGame.title}:`,
            error.message
          );
          errors.push({
      ...
      console.log("✅ Game synchronization completed:");
      console.log(`   📊 Total games processed: ${apiGames.length}`);
      console.log(`   🆕 New games added: ${newGamesCount}`);
      ...
      return {
        success: true,
        stats: {
          totalProcessed: apiGames.length,
      ...
      console.error("💥 Game synchronization failed:", error.message);
      return {
        success: false,
